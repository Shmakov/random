(function(){
    ...
    const app = {
        ...
        init: function() {
            document.getElementById('data').addEventListener('input', function (evt) {
                app.process_data();
            });
            app.process_data();
        },
        ...
        process_data: function() {
            ...
            lines.forEach(function (item, index) {
                [prev_from, prev_to, results[index]] = app.process_line(lines[index], [prev_from, prev_to]);
            });

            let html = results.join('');
            if (app.total_minutes > 0) {
                html += '<div class="total">Total: ' + convert_minutes_to_hours_minutes(app.total_minutes) + ' (' + app.total_minutes + ' minutes)</div>';
            }

            document.getElementById('results').innerHTML = html;
        },

        process_line: function(line, prev) {
            ...
            if (prev_to !== null && parseInt(from) < parseInt(prev_to)) {
                ...
                val_class+= ' correct ';
                minutes = app.calculate_minutes(from, to) + ' <span>minute(s)</span>';
            }
        ...
        calculate_minutes: function (_from, _to) {
            const from = moment()
